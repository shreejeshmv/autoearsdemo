...
class TestTaskModel:
    ...
    def test_is_overdue_uses_today_by_default(self) -> None:
        """When no explicit today is passed, is_overdue uses date.today()."""
        # A far-past date should always be overdue for a pending task.
        task = Task(title="Ancient", due_date=date(2000, 1, 1), status="pending")
        assert task.is_overdue() is True

    def test_is_overdue_day_before(self) -> None:
        """A task due yesterday is overdue."""
        task = Task(title="Yesterday", due_date=date(2025, 5, 31), status="pending")
        assert task.is_overdue(today=date(2025, 6, 1)) is True
